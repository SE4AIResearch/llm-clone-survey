...
def batch_convert_bib_to_csv():
    results_folder = os.path.join(os.path.dirname(__file__), "..", "results")

    for filename in os.listdir(results_folder):
        if filename.endswith(".bib"):
            bib_path = os.path.join(results_folder, filename)
            print(f"📄 Converting: {filename}")
            try:
                acm_df, acm_bib_entries = parse_bib_file_with_entries(bib_path)
                output_path = os.path.join(
                    results_folder, filename.replace(".bib", ".csv")
                )
                acm_df.to_csv(output_path, index=False)
...
if __name__ == "__main__":
    batch_convert_bib_to_csv()

    # Parse the two uploaded .bib files
    acm_df, acm_bib_entries = parse_bib_file_with_entries(
        "../results/acm_export2024.07.02-16.14.bib"
    )
    sciencedirect_df, sciencedirect_bib_entries = parse_bib_file_with_entries(
        "../results/ScienceDirect_2024-06-26_17-18-04.bib"
    )
